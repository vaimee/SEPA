...
public class RoadExperiment extends SmartLightingBenchmark {
	...
	public RoadExperiment() throws SEPAProtocolException, SEPASecurityException, SEPAPropertiesException {
		super();
	}
	...
	public void subscribe() {
		// Road subscription
		for (int i = 0; i < roadSubscriptionRoads.length; i++)
			addRoadSubscription(roadSubscriptionRoads[i]);

		// Lamp subscriptions
		for (int i = 0; i < lampSubscriptionRoads.length; i++)
			for (int X = lampSubscriptionRoads[i][0]; X < lampSubscriptionRoads[i][1] + 1; X++)
				for (int Y = lampSubscriptionLamps[i][0]; Y < lampSubscriptionLamps[i][1] + 1; Y++)
					addLampSubscription(X, Y);
	}

	@Override
	public void runExperiment() {
		for (int road = 1; road < nRoads + 1; road++)
			updateRoad(road, new Integer(100));
	}

	@Override
	public void reset() {
		for (int road = 1; road < nRoads + 1; road++)
			updateRoad(road, new Integer(0));
	}
