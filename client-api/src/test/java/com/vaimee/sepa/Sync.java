...
public class Sync implements ISubscriptionHandler {
	...
	public synchronized void waitSubscribes(int total) {
		synchronized (subscribesMutex) {
			while (subscribes < total) {
				try {
					Logging.trace("Thread id "+Thread.currentThread().threadId()+ " waitSubscribes");
					subscribesMutex.wait();
					Logging.trace("Thread id "+Thread.currentThread().threadId()+ " awaken from waitSubscribes");
	...
	public synchronized void waitEvents(int total) {
		synchronized (eventsMutex) {
			while (events < total) {
				try {
					Logging.trace("Thread id "+Thread.currentThread().threadId()+ " waitEvents");
					eventsMutex.wait();
					Logging.trace("Thread id "+Thread.currentThread().threadId()+ " awaken from waitEvents");
				} catch (InterruptedException e) {
					throw new RuntimeException(e.getCause());
				}
			}
	...
	public synchronized  void waitUnsubscribes(int total) {
		synchronized (unsubscribesMutex) {
			while (unsubscribes < total) {
				try {
					Logging.trace("waitUnsubscribes");
	...
	public void onBrokenConnection(ErrorResponse errorResponse) {
		if (errorResponse.getStatusCode() != 1000) Logging.error(errorResponse);
		else Logging.warn(errorResponse);
	}
