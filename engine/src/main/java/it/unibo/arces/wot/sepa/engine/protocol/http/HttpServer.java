...
public class HttpServer implements Runnable {
	...
	public HttpServer(EngineProperties properties,Scheduler scheduler) throws KeyStoreException, NoSuchAlgorithmException,
			CertificateException, IOException, UnrecoverableKeyException, KeyManagementException {

		// HTTP parameters for the server
		ConnectionConfig config = ConnectionConfig.DEFAULT;
		InetSocketAddress address = new InetSocketAddress(properties.getHttpPort());
		...
		HttpRequestHandler handler = new HttpRequestHandler(properties,scheduler);
		...
		connFactory = new DefaultNHttpServerConnectionFactory(config);
		...
		// Welcome message
		try {
			System.out.println("Query URL: http://" + InetAddress.getLocalHost().getHostAddress() + ":"
					+ address.getPort() + handler.getQueryPath());
		} catch (UnknownHostException e1) {
			logger.error(e1.getLocalizedMessage());
		}
		try {
			System.out.println("Update URL: http://" + InetAddress.getLocalHost().getHostAddress() + ":"
					+ address.getPort() + handler.getUpdatePath());
		} catch (UnknownHostException e1) {
			logger.error(e1.getLocalizedMessage());
		}
