...
public class WebAccessControlManager {
	...
	public PermissionSet handle ( String identifiers, PermissionSet permissions, String credentials ) {
		...
		// this.checkPermissions(permissions, allowedModes); NOT HERE
...
//	}
	 ...
	 * */
	private PermissionSet createAuthorization(String credentials, Model acl) {
		...
		if(this.hasPermission(acl, ACL + "Read", credentials, ruleList)) {
			permissions.changePermission("read", true);
		}
		
		if(this.hasPermission(acl, ACL + "Write", credentials, ruleList)) {
			permissions.changePermission("write", true);
			permissions.changePermission("append", true);
		}else if(this.hasPermission(acl, ACL + "Append", credentials, ruleList)) {
			permissions.changePermission("append", true);
		}
		
		if(this.hasPermission(acl, ACL + "Control", credentials, ruleList)) {
			permissions.changePermission("control", true);
		}
	...
	private Model getAclRecursive (String id, boolean recurse) {
		...
		*/
		...
		model.createResource(aclPrefix + "#auth1")
		.addProperty(RDF.type, aclAuthorization)
		.addProperty(aclAgent, userTestAgent)
		.addProperty(aclMode, aclRead)
		.addProperty(aclMode, aclWrite)
		.addProperty(aclMode, aclAppend)
		.addProperty(aclMode, aclControl)
		.addProperty(aclAccessTo, resource);
		
		model.createResource(aclPrefix + "#auth2")
		.addProperty(RDF.type, aclAuthorization)
		.addProperty(aclAgentGroup, testGroup)
		.addProperty(aclMode, aclRead)
		.addProperty(aclMode, aclWrite)
		.addProperty(aclAccessTo, resource);
		
		model.createResource(aclPrefix + "#auth3")
		.addProperty(RDF.type, aclAuthorization)
		.addProperty(aclAgentClass, FOAF.Agent)
		.addProperty(aclMode, aclRead)
		.addProperty(aclAccessTo, resource);
