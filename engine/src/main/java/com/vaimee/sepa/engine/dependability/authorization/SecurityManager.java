...
//			RSAKey jwk = RSAKey.load(keystore, keyAlias, keystorePwd.toCharArray());
	...
	private void setupValidation(RSAKey jwk) throws JOSEException {		
		...
		Logging.getLogger().debug("Public key to validate JWT");
		Logging.getLogger().debug(jwkPublicKey);
	...
	public void securityCheck(String identity) throws SEPASecurityException {
		Logging.getLogger().info("*** Security check ***");
		// Add identity
		...
		if (response.getClass().equals(RegistrationResponse.class)) {
			...
			Logging.getLogger().debug("Authorization Basic " + encodedCredentials);
			response = getToken(encodedCredentials);

			if (response.getClass().equals(JWTResponse.class)) {
				Logging.getLogger().debug("Access token: " + ((JWTResponse) response).getAccessToken());
				...
				if (authRet.isAuthorized()) {
					...
					Logging.getLogger().error(authRet.getError());
					Logging.getLogger().info("*** FAILED ***");
				}
			} else {
				Logging.getLogger().debug(response.toString());
				Logging.getLogger().info("*** FAILED ***");
			}
		} else {
			Logging.getLogger().debug(response.toString());
	...
	public synchronized Response register(String uid) {
		Logging.getLogger().info("REGISTER: " + uid);

		// Check if entity is authorized to request credentials
		try {
			if (!isAuthorized(uid)) {
				Logging.getLogger().warn("Not authorized identity " + uid);
	...
	public synchronized Response getToken(String encodedCredentials) {
		...
		try {
			...
			Logging.getLogger().error("Not authorized");
		...
		if (marker == -1) {
			Logging.getLogger().error("Wrong Basic authorization");
...
//		String secret = decodedCredentials.split(":")[1];
		...
		Logging.getLogger().debug("Credentials: " + id + " " + secret);

		// Verify credentials
		try {
			if (!containsCredentials(id)) {
				Logging.getLogger().error("Client id: " + id + " is not registered");
		...
		try {
			if (!checkCredentials(id, secret)) {
				Logging.getLogger().error("Wrong secret: " + secret + " for client id: " + id);
		...
		try {
			if (containsJwt(id)) {
				...
				Logging.getLogger().debug("ID: " + id + " ==> Token will expire in: " + delta + " ms");
				if (delta > 0) {
					Logging.getLogger().warn("Token is NOT EXPIRED. Return the current token.");
				...
				Logging.getLogger().debug("Token is EXPIRED. Release a fresh token.");
			}
		} catch (SEPASecurityException e2) {
...
//		claimsSetBuilder.audience(audience);
		...
		Logging.getLogger().debug("Released token: " + jwt);
	...
	public synchronized ClientAuthorization validateToken(String accessToken) {
		Logging.getLogger().debug("VALIDATE TOKEN");
		...
		try {
			if (!signedJWT.verify(verifier)) {
				Logging.getLogger().error("Signed JWT not verified");
		...
		if (expiring.getTime() - nowUnixSeconds < 0) {
			Logging.getLogger().warn("Token is expired: " + sdf.format(claimsSet.getExpirationTime()) + " < "
		...
		if (notBefore != null && nowUnixSeconds < notBefore.getTime()) {
			Logging.getLogger().warn("Token can not be used before: " + claimsSet.getNotBeforeTime());
		...
		Logging.getLogger().debug("Get credentials for uid: "+id);
		
		Credentials cred = null;
		try {
			cred = getEndpointCredentials(id);
			Logging.getLogger().trace(cred);
		} catch (SEPASecurityException e) {
			Logging.getLogger().error("Failed to retrieve credentials ("+id+")");
